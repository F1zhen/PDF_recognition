...
3️⃣ Результаты сохраняются автоматически
...
3️⃣ Детектор QR / штрихкода (QrDetector)

YOLO-модель для QR и barcode.

4️⃣ Ensemble-агрегатор (EnsembleDetector)
...
⚙️ Backend / ML: установка зависимостей
...
Активировать виртуальное окружение (см. выше).

Перейти в директорию backend/ (если есть отдельная папка) или в корень проекта, где лежит main.py FastAPI-приложения.
...
По умолчанию фронт будет доступен, например, на http://localhost:5173 (или другом порту, который выдает Vite/Next и т.д., в зависимости от стека).
...
(При необходимости можно добавить сюда любые доп. библиотеки, которые используются во фронте/бэке.)
